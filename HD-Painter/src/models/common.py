...
def load_sd_inpainting_model(
    ...
    base_folder = cache_dir if cache_dir is not None else MODEL_FOLDER
    ...
    if type(download_url) == str and type(model_path) == str:
        ...
        for key in download_url.keys():
            download_file(download_url[key], f'{base_folder}/{model_path[key]}')
        unet_state = load_state_dict(f'{base_folder}/{model_path["unet"]}')
        encoder_state = load_state_dict(f'{base_folder}/{model_path["encoder"]}')
        vae_state = load_state_dict(f'{base_folder}/{model_path["vae"]}')
        if diffusers_ckpt:
