...
def main():
    ...
    if not os.path.exists(RECORDS_PATH):
        print(f"records.pkl not found: {RECORDS_PATH}")
        return
    ...
    for idx, layer_idx in enumerate(LAYERS):
        ...
        print(f"  Clean:      mean={all_orig.mean():.4f}, std={all_orig.std():.4f}")
        print(f"  Perturbed:  mean={all_wm.mean():.4f},   std={all_wm.std():.4f}")

        if zero_mode:
            print(f"  After:      mean={all_regen.mean():.4f}, std={all_regen.std():.4f}")
            ...
            fg_vals = all_regen[fg_tile]
            bg_vals = all_regen[bg_tile]
            print(f"  Foreground: mean={fg_vals.mean():.4f}, std={fg_vals.std():.4f}")
            print(f"  Background: mean={bg_vals.mean():.4f}, std={bg_vals.std():.4f}")

            ax.hist(all_orig, bins=bins, color='darkgray', label='Clean', 
                    density=True, histtype='step', linewidth=2.0, linestyle=':')
            ax.hist(all_wm, bins=bins, color='darkgray', label='Perturbed', 
                    density=True, histtype='step', linewidth=1.5, linestyle='--')
            ax.hist(fg_vals, bins=bins, color='tomato', label='Foreground', 
                    density=True, alpha=0.55)
            ax.hist(bg_vals, bins=bins, color='steelblue', label='Background', 
                    density=True, alpha=0.55)
