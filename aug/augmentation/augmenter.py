...
class Augmenter(nn.Module):
    ...
    def post_augment(self, images, masks, do_resize=True):
        """
        images: Tensor [B, C, H, W]
        masks:  Tensor [B, 1, H, W] 或 [B, H, W]
        """
        B = images.shape[0]
        auged_images, auged_masks, aug_names = [], [], []

        for i in range(B):
            image, mask = images[i:i+1], masks[i:i+1]  # 保留batch维度方便插值
            index = torch.multinomial(self.post_probs, 1).item()
            selected_aug = self.post_augs[index]
            ...
            if not do_resize:
                ...
                if image.shape[-2:] != (h, w):
                    image = nn.functional.interpolate(
                        image, size=(h, w), mode='bilinear', 
                        align_corners=False, antialias=True
                    )
                    mask = nn.functional.interpolate(
                        mask, size=(h, w), mode='nearest'
                    )
            auged_images.append(image)
            auged_masks.append(mask.int())
            aug_names.append(str(selected_aug))

        auged_images = torch.cat(auged_images, dim=0)
        auged_masks = torch.cat(auged_masks, dim=0)
        return auged_images, auged_masks, aug_names
